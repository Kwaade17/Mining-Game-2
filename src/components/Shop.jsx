...
import { bagShop, energyShop } from "../assets/shopData"
...
export default function Shop({player, setPlayer}) {
  ...
  return(
    <div className="w-full h-full flex flex-col justify-center items-center p-4 space-y-4 overflow-hidden bg-gray-900">
      <div className="flex flex-col w-full p-4 rounded-xl bg-gray-800">
        <span className="px-2 mb-4 font-bold text-white text-xl sm:text-4xl">
          Energy Shop
        </span>
        <div
          ref={energyScrollRef}
        ...
        >
          {energyShop.map((item, index) => (
            <div key={`${item.name}-${index}`} className="w-48 shrink-0 mr-4 p-4 space-y-4 text-center flex flex-col justify-center items-center rounded-xl text-white bg-gray-700">
              <span className="w-full p-1 rounded-xl font-bold text-white bg-gray-800">
                {item.name}
              </span>
              <div className="w-14 h-14 p-2 rounded-full bg-gray-800">
                <svg
              ...
              <span className="w-full rounded-xl px-4 py-2 text-sm bg-gray-800">
                {item.details}
              </span>
              <button
                onClick={() => {
                ...
                className="w-full rounded-xl p-1 mt-1 font-bold text-lime-800 bg-lime-400 hover:bg-lime-300 cursor-pointer">
                $ {item.price}
              </button>
            </div>
          ))}
      ...
      <div className="flex flex-col w-full p-4 rounded-xl bg-gray-800">
        ...
        >
          {bagShop.map((item, index) => (
            <div key={`${item.name}-${index}`} className="w-48 shrink-0 p-4 space-y-4 text-center flex flex-col justify-center items-center rounded-xl text-white bg-gray-700">
              <span className="w-full p-1 rounded-xl font-bold text-white bg-gray-800">
                {item.name}
              </span>
              <div className="w-14 h-14 p-2 rounded-full bg-gray-800">
              ...
              <button
                ...
                className={`w-full rounded-xl p-1 mt-1 font-bold ${item.isBought ? "bg-slate-500 cursor-not-allowed" : "text-lime-800 bg-lime-400 hover:bg-lime-300 cursor-pointer"}`}>
